...
impl IndexStore {
    pub fn new(tik_root: PathBuf) -> IndexStore {
        IndexStore { tik_root }
    }
    ...
    pub fn rebuild(
        ...
        Ok(IndexSummary {
            indexed_at: timeutil::now_rfc3339()?,
...
fn build_search_sql(
    ...
    push_in_clause("priority", &filters.priorities, &mut conditions, &mut params);
    push_in_clause("severity", &filters.severities, &mut conditions, &mut params);
    push_in_clause("milestone_id", &filters.milestone_ids, &mut conditions, &mut params);
...
mod tests {
    ...
    fn rebuild_and_search_index() {
        ...
        tikfs::ensure_dir(&tik_root).unwrap();
        ...
        let store = IndexStore::new(tik_root);
        let summary = store.rebuild(&[ticket.clone()], &notes).unwrap();
    ...
    fn list_ids_respects_offset_and_limit() {
        ...
        ticket_a.id = crate::domain::ids::TicketId::parse("T-01ARZ3NDEKTSV4RRFFQ69G5FAV")
            .unwrap();
        ...
        ticket_b.id = crate::domain::ids::TicketId::parse("T-01ARZ3NDEKTSV4RRFFQ69G5FAW")
            .unwrap();
        ...
        let store = IndexStore::new(tik_root);
        store.rebuild(&[ticket_a.clone(), ticket_b.clone()], &notes)
            .unwrap();
